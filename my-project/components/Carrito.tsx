...
const totalSum = cartItems.reduce((total, item) => total + item.price, 0).toFixed(2);
  ...
  return (
    <div>
        <button onClick={()=> setOpenModal(true)}  className="flex items-start">
                <p className='absolute text-white pl-3'>{quantityItems}</p>
        ...
        >
          <div className='font-mono'>
            <h2 className=' text-gray-900 text-lg pb-2'>Productos añadidos al carrito: {quantityItems} </h2>
            <div className='flex flex-col gap-3'>
            {cartItems.length > 0 ? (
            cartItems.map((item) => (
                <div key={item.id} className='border bg-slate-200 border-s-4 '>
                  <h3 className='flex justify-center border' >{item.name}</h3>
                  <img className='w-9/12' src={item.src} alt={item.alt} />
                  <div className='flex justify-between'>
                    <div className='flex flex-col pl-2 pt-2'>
                      <p className='font-semibold '>
                        Precio: <span className='text-green-600'>${item.price}</span>
                      </p>                    
                      <span className='pr-4 '>Cantidad: {}</span>   
                    </div>
                    <div className='flex justify-end p-3'>
                      <button className=' bg-red-500 hover:bg-red-700 text-white font-bold-500 py-1 px-2 rounded' onClick={() => removeItem(item.id)}>
                        <svg className="h-4 w-4 text-white-200"  fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <p className='pt-4 border-b-2'>Total: ${totalSum}</p>
          </div>
        </Modal>
    </div>
  )
